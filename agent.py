...
class ValueIterationAgent(Agent):
  ...
  def __init__(self, mdp, discount = 0.9, iterations = 100):
    ...
    self.values = { i : 0 for i in mdp.getStates() }
    print("ddddddddddddddddddddddd",mdp.getStartState())
      ...
      for state in self.mdp.getStates():
        ...
        for action in self.mdp.getPossibleActions(state):
          ...
          for sPrime, prob in self.mdp.getTransitionStatesAndProbs(state, action):
            actionVal += prob * (self.mdp.getReward(state, action, None) + self.discount * self.values[sPrime])
          if (actionVal > maxA):
            maxA = actionVal
        if (maxA > float('-inf')):
          uPrime[state] = maxA
  ...
  def getQValue(self, state, action):
    ...
    for sPrime, prob in self.mdp.getTransitionStatesAndProbs(state, action):
      noSuccessors = False
      ret += prob * (self.mdp.getReward(state, action, sPrime) + self.discount * self.getValue(sPrime))
    if noSuccessors:
      ret = None
    return ret
